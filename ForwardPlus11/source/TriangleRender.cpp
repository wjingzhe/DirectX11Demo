...
namespace Triangle
{
	...
	void TriangleRender::GenerateMeshData(float width, float height, float depth, unsigned int numSubdivision, float centerX, float centerY, float centerZ)
	{
		m_MeshData = GeometryHelper::CreateBox(width, height, depth, numSubdivision, centerX, centerY, centerZ);
	}
		...
		ID3D11RenderTargetView* pRTV, ID3D11DepthStencilView* pDepthStencilView)
	{
		...
		pD3dImmediateContext->RSSetState(m_pCullingBackRS);
