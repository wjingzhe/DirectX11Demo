...
Texture2D<float> tCircleOfConfusion:register(t1);
RWTexture2D<float> uCircleOfConfusion:register(u0);
RWTexture2D<float4> uDebugVisCircleOfConfusion:register(u1);
...
[numthreads(8,8,1)]
void CalcDOF(uint3 threadID:SV_DispathThreadID)
{
	if ((threadID.x < ScreenParams.x) && (threadID.y < ScreenParams.y))
	{
		...
		float circleOfConfusion = clamp(CocFromDepth(camDepth, focusDistance, fStop, focalLength), -maxRadius, maxRadius);
		...
		uCoc[int2(threadID.xy)] = circleOfConfusion;
